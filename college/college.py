...
class Teacher(Human):
    ...
    def __str__(self):
        if self.patronymic:
            return f'учитель : {self.last_name} {self.first_name[0]}. {self.patronymic[0]}. Преподаёт: {self.experience} ({self.education})'
        else:
            return f'учитель : {self.last_name} {self.first_name[0]}. Преподаёт: {self.experience} ({self.education})'